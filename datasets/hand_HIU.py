...
class hand_HIU(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path, msk_path, lbl_path = osp.join(self.dataset_folder, index+'.jpg'), osp.join(self.dataset_folder, index+"_mask.png"), osp.join(self.dataset_folder, index+".json")
        image,  mask, label = cv2.imread(img_path), cv2.imread(msk_path)[...,0], json.load(open(lbl_path, 'r'))
        key_points_2d, left = np.array(label['key_point_2ds'][:21]), label['left']
        if not left:
            key_points_2d[:, 0] = image.shape[1]-1-key_points_2d[:, 0]
            image, mask = image[:, ::-1].copy(), mask[:, ::-1].copy()
        if key_points_2d.shape[0]!=NUM_KPS:
            return self.__getitem__(np.random.randint(0, len(self)))
            
        index_visible = (key_points_2d[:, 2]>=1)
        key_points_2d[index_visible, 2] = 1
        
        key_points_3d, mano_theta, mano_beta = np.ones([NUM_KPS, 4]), np.zeros([46]), np.zeros([11])
