...
class hand_cmu(data.Dataset):
    ...
    def load_dataset(self, dataset_folder):
        lst_path = osp.join(dataset_folder, 'hand_labels', 'train.lst' if self.mode=='train' else 'val.lst')
        with open(lst_path, 'r') as fp:
            for s_sp in fp.readlines():
                self.samples.append('hand_labels/'+s_sp.replace('\n', ''))
    ...
    def __getitem__(self, index):
        ...
        json_path, img_path = osp.join(self.dataset_folder, index+'.json'), osp.join(self.dataset_folder, index+'.jpg')
        label = json.load(open(json_path, 'r'))
        key_points_2d, is_left = np.array(label['hand_pts']), label['is_left']
        ...
        mask = key_points_2d[:, 2]==0
        key_points_2d[mask, 2] = 4
