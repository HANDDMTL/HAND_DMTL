...
class hand_STB(data.Dataset):
    ...
    def load_dataset(self, dataset_folder):
        lst_path = osp.join(dataset_folder, 'train.lst' if self.mode=='train' else 'eval.lst')
    ...
    def __getitem__(self, index):
        ...
        key_points_3d, key_points_2d, img_name, is_left = np.array(lbl['key_points_3d']), np.array(lbl['key_points_2d']), lbl['img_name'], lbl['is_left']

        key_points_2d[:, 2] = 4.0*key_points_2d[:, 2]
